...
<div class="max-w-3xl animate-fade-in-up animate-delay-1">
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-md p-6 md:p-8">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div class="mb-1">
                ...
                <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price', $product->sale_price) }}" class="input-field" placeholder="0.00">
            ...
            <div class="mb-1">
                ...
                <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="input-field" placeholder="e.g. PROD-001">
        ...
        <div class="mb-4">
            ...
            @if($product->images)
                <p class="text-xs text-gray-400 mt-1">{{ count($product->images) }} image(s) currently uploaded</p>
            @endif
            @error('images.*')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
        </div>
        <div class="mb-6">
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
